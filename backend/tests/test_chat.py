...
def test_chat_single_model_spiderman(client):
    ...
    with patch("main.gemini_client.generate_single", new_callable=AsyncMock) as mock_gen:
...
def test_chat_compare_mode(client):
    ...
    with patch("main.gemini_client.generate_compare", new_callable=AsyncMock) as mock_comp:
...
def test_chat_streaming_mode(client):
    ...
    with patch("main.gemini_client.stream_single", side_effect=mock_stream):
...
def test_chat_gemini_unreachable_error(client):
    """Test /api/chat returns 503 when Gemini client raises GeminiServiceError."""
    with patch("main.gemini_client.generate_single", side_effect=GeminiServiceError("Gemini service unreachable", 503)):
        response = client.post("/api/chat", json={"message": "Who is Peter?", "model": "spiderman", "compare": False})
        assert response.status_code == 503
        assert "Gemini service unreachable" in response.json()["detail"]

def test_chat_timeout_error(client):
    """Test /api/chat returns 504 when Gemini generation times out."""
    with patch("main.gemini_client.generate_single", side_effect=GeminiServiceError("Model generation timed out (>60s).", 504)):
        response = client.post("/api/chat", json={"message": "Who is Peter?", "model": "spiderman", "compare": False})
